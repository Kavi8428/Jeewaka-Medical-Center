...
formattedData.forEach(data => {
    ...
    row.innerHTML = `
        ...
        <td class="text-center">${data.NPAID}</td>
        <td class="text-end">${data.JMC}</td>
        <td class="text-end">${data.JD}</td>
...
document.getElementById('docNP').textContent = totalNP.toFixed(2);
document.getElementById('docJMC').textContent = totalJMC.toFixed(2);
document.getElementById('docJD').textContent = totalJD.toFixed(2);
document.getElementById('docTotal').textContent = grandTotal.toFixed(2);
...
.catch(error => {
    ...
    document.getElementById('docReportPDF').addEventListener('click', function() {
        ...
        html2canvas(table, {
            ...
            // Add generated date and time at the bottom of the last page
            doc.text(formattedDateTime, 40, doc.internal.pageSize.height - 30);

            today = new Date();
            const currentDate = new Date().toISOString().split('T')[0];
                    const fileName = `Doc Cash Report ${currentDate}.pdf`;
                    doc.save(fileName);
    ...
    function filterTable() {
        fetch("./attributes/fetchPatientDeta.php")
            ...
            .then(patientData => {
                ...
                // Group data by date and nurse, while tracking separate totals for cash, card, and bank
                const groupedData = {};
                filteredData.forEach(entry => {
                    const entryDate = new Date(entry.updated_at);
                    const nurse = entry.cashNurse; // Assuming 'cashNurse' contains nurse information
                    const paymentMethod = entry
                        .paymentMethod; // Assuming 'paymentMethod' contains payment method information
                    const year = entryDate.getFullYear();
                    const month = ('0' + (entryDate.getMonth() + 1)).slice(-
                        2); // Adding 1 because getMonth() returns zero-based month index
                    ...
                    if (paymentMethod === 'cash&card') {
                        groupedData[dateString][nurse].cash += received -
                            cardValue; // Add received value (excluding card part) to cash
                        groupedData[dateString][nurse].card += cardValue; // Add card value to card
        ...
        fetch("./attributes/fetchPatientDeta.php")
            ...
            .then(data => {
                ...
                const filteredData = patientData.filter(patient => {
                    if (!patient || !patient.serial || !patient.created_at) {
                ...
                serialNumbers.forEach(serialNumber => {
                    ...
                    li.addEventListener('click', function() {
                        ...
                        selectedPatient = filteredData.find(patient => patient.serial ===
                            serialNumber);
                        if (selectedPatient) {
