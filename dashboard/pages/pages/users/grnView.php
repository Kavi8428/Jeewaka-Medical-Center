...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3   bg-gradient-dark" id="sidenav-main">
            ...
            <li class="nav-item mb-2 mt-0">
              ...
              <div class="collapse" id="ProfileNav" style="">
                ...
                <li class="nav-item" <?php if($fullName != 'RAVI  NANAYAKKARA'){echo 'hidden';} ?>>
                  ...
                  <li class="nav-item">
                    <a class="nav-link text-white active " href="./grnView.php">
              ...
              <div class="d-lg-flex">
                ...
                <div class="ms-auto  my-auto mt-lg-0 mt-4">
                  <div class="ms-auto my-auto ">
                   ...
                   <button type="button" id="export" class="btn bg-gradient-info btn-sm mb-0">DOWNLOAD</button>
  ...
  if (document.getElementById('products-list')) {
    fetch("../../../../attributes/fetchGrnProducts.php")
      ...
      .then(grnProduct => {
        const grnGroups = grnProduct.reduce((groups, row) => {
          if (!groups[row.grnNo]) {
            groups[row.grnNo] = 0;
          }
          groups[row.grnNo] += parseFloat(row.tc);
          return groups;
        }, {});
        ...
        fetch("../../../../attributes/fetchGrnData.php")
          ...
          .then(data => {
            ...
            const createTable = (filteredData) => {
              ...
              window.dataTableInstance = new simpleDatatables.DataTable("#products-list", {
                data: {
                  headings: ['GRN NO', 'IV NUMBER', 'COMPANY', 'MRBONUS', 'PAYMENT', 'P_VALUE', 'DATE'],
                  data: filteredData.map(row => {
                    let correctedStatus;
                    if (row.status === 'approved') {
                      correctedStatus = 'COMPLETED';
                    } else if (row.status === 'pending') {
                      correctedStatus = 'NOT ADDED';
                    } else if (row.status === 'addMrToStock') {
                      correctedStatus = 'COMPLETED';
                    } else {
                    ...
                    if (row.payment === 'done') {
                      correctedPayment = 'DONE';
                    } else {
                      correctedPayment = '';
                    }

                    const totalTc = grnGroups[row.id] || 0;
              ...
              const dataTableWrapper = document.querySelector('.dataTable-top .dataTable-search');
              if (dataTableWrapper) {
                dataTableWrapper.style.display = 'flex';
                dataTableWrapper.style.alignItems = 'center';

                if (!document.getElementById('companyFilterContainer')) {
                  const companyFilterContainer = document.createElement('div');
                  companyFilterContainer.id = 'companyFilterContainer';
                  companyFilterContainer.style.marginRight = '10px';
                  companyFilterContainer.appendChild(companyFilter);
                  dataTableWrapper.prepend(companyFilterContainer);
                }
              }
            };
            ...
            document.getElementById('export').addEventListener('click', function() {
              ...
              const rows = [];
              let totalPValue = 0; // Initialize total P_VALUE

              document.querySelectorAll('#products-list tbody tr').forEach(tr => {
                const row = [];
                tr.querySelectorAll('td').forEach((td, index) => {
                  row.push(td.textContent);
                  // Add P_VALUE to total if it's the correct column (P_VALUE column index is 5)
                  if (index === 5) {
                    totalPValue += parseFloat(td.textContent);
                  }
                });
                rows.push(row);
              });

              // Add total P_VALUE row
              rows.push(['', '', '', '', 'Total P_VALUE', totalPValue.toFixed(2), '']);

              doc.autoTable({
                head: [['GRN NO', 'IV NUMBER', 'COMPANY', 'MRBONUS', 'PAYMENT', 'P_VALUE', 'DATE']],
                body: rows,
                foot: [['', '', '', '', 'Total P_VALUE', totalPValue.toFixed(2), '']]
              });

              const currentDate = new Date().toISOString().split('T')[0];
              const fileName = `GRN Report ${currentDate}.pdf`;
              doc.save(fileName);
            });
          })
          .catch(error => {
      ...
      .catch(error => {
        console.error('Error fetching the GRN products:', error);
      });
  }
});
